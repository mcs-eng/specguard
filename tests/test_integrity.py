...
def test_a_rebuilt_fixture_screens_identically(tmp_path: Path) -> None:
    """A rebuild of the altered fixture yields the same screen report."""
    rebuilt = {path.name: path for path in build_fixtures(tmp_path)}
    committed = check_text_layer(ALTERED_FIXTURE)
    fresh = check_text_layer(rebuilt[ALTERED_FIXTURE.name])

    assert fresh.sha256 == committed.sha256
    assert fresh.flagged_pages == committed.flagged_pages
    assert [span.text for span in fresh.hidden_spans] == [
        span.text for span in committed.hidden_spans
    ]
