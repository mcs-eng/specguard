...
- Google ADK 2.7.1: one `LlmAgent` with `output_schema` for the claim batch, in either of two modes. `full_text`, the deployed default, sends both documents up front. `navigate` sends the submitted document in full plus a page index of the specification and registers three read-only tools (`check_text_integrity`, `extract_pdf_text`, `verify_quote`), each bound to a document role rather than a path; the two write tools (`persist_finding`, `draft_rfi`) are not registered in that mode. In both modes every model-initiated call is recorded on the run and shown on its page, the runtime owns every write, and the gate verifies every claim before it reaches the ledger. Which mode ships was decided by a pre-registered eight-condition gate, not by preference; both are published in EVAL.md.
...
All numbers from `EVAL.md` (100 real audits on 2026-08-25 against code revision `31ef186`, both agent modes, every run identifier listed) and `HANDOFF.md`. They supersede the 2026-08-23 five-iteration measurement, which ran under a stricter scoring rule and against an earlier package fixture.
...
- Messy lane (a 32-page specification and a 10-page two-product package with seven planted discrepancies and two compliant decoys): full text caught 96% against navigate's 94%, with 0 decoy false positives and 0 unattributed false positives in either mode. One planted case carries almost the whole gap: E-15 measures 80% in full text and 60% in navigate. Every other planted case on that lane measures 90% or 100%.
...
- The default follows an eight-condition ship gate fixed in writing before any run and evaluated by a pure function over the results. Navigate met seven conditions and missed one by a single case-run in seventy, so full text ships and navigate stays selectable and published. The rule was pre-registered precisely so a two-point gap could not be argued either way after the fact.
