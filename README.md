...
The agent proposes; the runtime disposes. One Google ADK agent on Gemini 3.7 Flash reads the two documents and returns discrepancy claims, each with one verbatim quote and one page number per document. A deterministic runtime around the agent screens both documents before the model reads anything, verifies every quoted anchor, sends a rejected claim back to the model once, verifies again at write time, and drafts the RFI. The runtime has two modes, selected by `SPECGUARD_AGENT_MODE`. In `full_text`, the deployed default, the model receives every page of both documents up front. In `navigate`, the model receives the submitted document in full plus a one-line-per-page index of the specification and reads specification pages through three read-only tools; every model-initiated tool call is recorded on the run and shown on its page. In both modes the runtime owns every write and the gate verifies every claim, whatever the model called. Which mode ships was decided by an eight-condition gate written into the phase work order before the measurement ran, and evaluated by a pure function over the results. Navigate met seven conditions and failed one: on the hardest fixture lane it caught 94 percent against full text's 96 percent, which is one case-run in seventy. That margin is noise rather than a capability gap, and the gate is deliberately written to follow a measurement instead of a judgement call, so full text is the default. Navigate stays selectable and fully receipted, and [EVAL.md](EVAL.md) publishes both modes with the verdict line by line. Navigate is the receipted architecture for a specification too large to send in one prompt, which is a case these fixtures never created: the largest pair here fits in a single request. Navigate also costs more prompt tokens than full text, roughly 30k against 13k per run on the original lane and 312k against 259k on the messy lane.
...
**Provenance of the current table.** The table above was regenerated on 2026-08-25 by the harness at code revision `31ef186`, over 100 real audits at ten iterations per document pair. The measured revision is the described revision. The 2026-08-23 measurement at five iterations on revision `1340748` is superseded by this campaign, not corrected by it: it ran at a different iteration count and under a different scoring rule, so the two records are not comparable cell by cell. The generated table states, in its own last line, which cases moved.

**What moved since the 2026-08-23 run, and why.** Two deliberate changes landed before this campaign and both move numbers upward, so neither is a model improvement. The harness now scores a catch by evidence equality: the finding must cite the page the manifest records on each side, and the persisted quote and the manifest quote must contain one another in either direction after the gate's own normalization. The earlier rule required the persisted span to equal the planted span byte for byte, which scored a correct finding with a shorter quote as a miss and then counted the same record as an unattributed false positive. The `M-05` package row was also re-specified from a disclaimed recommendation to a flat characteristic of the product as furnished, which turned `E-15` from a case the prompt's own both-can-be-true exclusion covered into a case the model can fairly be asked to report. Both changes are receipted below. The severity column is unchanged in meaning: this run again executed with the severity sentinel (`SPECGUARD_GEMMA_ENDPOINT=disabled`), so every persisted finding carries `unclassified` with its recorded fallback reason.
...
**The messy-lane misses that remain.** `E-13` and `E-15` were published at 40 percent and 0 percent by the 2026-08-23 run. Phase 7d inspected both against the persisted records of those runs and found two different causes, one in the scorer and one in the planted text. Both causes were corrected before this campaign, and both cases now measure as real detection results rather than as artefacts. The inspection and every receipt behind it are in [REVIEW-7D-MISSES.md](REVIEW-7D-MISSES.md); it describes the earlier run and is not a record of this one.

- `E-13` was a scoring artefact and is now measured as a catch. Under the old rule six of ten runs that found the discrepancy on the correct pages were scored as misses for returning a shorter quote span. Under evidence equality the same behaviour scores as what it is: `E-13` measures 100 percent in `navigate` and 90 percent in `full_text` across the ten runs of each mode.
- `E-15` was a fixture-design finding and is now a real test the model sometimes fails. No run in either mode had ever raised that pair, because the package stated its clearance figure as a recommendation while both prompts tell the model not to report statements that can both be true. The package row now states a flat characteristic of the product as furnished, which is the shape of the six pairs that already caught. Nothing else on that page changed, and no prompt changed. `E-15` measures 80 percent in `full_text` and 60 percent in `navigate`, which are the two lowest case rates on the board and are published as measured.
...
| 7c | `full_text` mode registers the two write tools, `persist_finding` and `draft_rfi`, to the model, and `full_text` is now the deployed default. | OPEN — narrowing `full_text` to the same three read-only tools `navigate` registers is decided but not yet applied, because the receipt that would prove the change neutral for the published numbers does not exist: the eval harness writes no `runs` document, so the campaign persisted no `model_tool_calls` receipt for any of its 50 `full_text` runs. What the persisted records do show is that no model-initiated write succeeded — Firestore holds exactly 87 findings for those runs against the runtime's own count of 87, with zero rejection records. Every write path runs the gate at write time and again before the write, so an unverified claim cannot enter the ledger by either route; recorded in `HANDOFF.md` Phase 7d-remeasure. |
