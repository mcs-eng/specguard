...
| P6e review | Every run page GET downloaded and reparsed both stored PDFs, with no cache. | FIXED — `<fix-commit>` caches one window set per run, keyed by a digest of the anchors it was built from, so a reload reads nothing and a run still writing findings recomputes. A failed read is never cached. |
| P6e review | A findings-table row taller than one RFI page was clipped, not split. | FIXED — `<fix-commit>` writes such a row in slices under a repeated header, and moves a row whole only when a fresh page would actually hold it. |
