...
`uv run pytest -q` exited 0 with **659 passed** on code revision `ce605ec`. This line is written by `scripts/record_test_count.py` from that run's own summary line; it is not typed by hand. No test requires the network or credentials; every model, Firestore, and storage dependency is an in-process fake.
...
| 7c | `E-15` (0 of 5) and `E-13` (2 of 5) were published misses on the messy lane in both modes. | CLOSED — both causes were corrected and both cases re-measured at N=10. `E-13` was a scoring artefact and now measures 100% in `navigate` and 90% in `full_text` under evidence-equality scoring. `E-15` was a fixture-design finding and now measures 60% and 80% against a package row that states a flat product characteristic. `E-15` remains the lowest case on the board and publishes as an honest miss on a now-fair test; disclosed in [Measured evaluation](#measured-evaluation). The earlier inspection stays receipted in `REVIEW-7D-MISSES.md`, which describes the superseded run. |
