...
Measured on 2026-08-22 by `scripts/eval_fixtures.py`, 5 runs per case against the deployed Vertex AI model path, on code revision `8854969`. Catch rate: 100% across every planted discrepancy. Every case matched its declared expected outcome in every run. Column definitions and the full record are in [EVAL.md](EVAL.md).
