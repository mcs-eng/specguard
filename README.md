...
**The two modes.** `SPECGUARD_AGENT_MODE` selects them. In `full_text`, the deployed default, the model receives every page of both documents up front. In `navigate`, the model receives the submitted document in full plus a one-line-per-page index of the specification and reads specification pages through read-only tools. Both modes register the same three read-only tools and neither of the two that write, so the model reads in either mode and writes in neither. The runtime owns every write and the gate verifies every claim, whatever the model called. `navigate` is the receipted architecture for a specification too large to send in one prompt, a case these fixtures never created: the largest pair here fits in a single request. It also costs more prompt tokens. Measured in the 2026-08-25 campaign at code revision `8900a76`, `navigate` averaged 31,750 prompt tokens per run against full text's 7,435 on the original lane, and 274,535 against 152,425 on the messy lane.

**Which mode is the default, and how that closed.** An eight-condition ship gate, written into the phase work order before any measurement ran and evaluated by a pure function over the results, was the decision procedure for the mode question. The first evidence-equality campaign, on 2026-08-25 at code revision `31ef186` and ten audits per document pair, failed one of the eight: on the messy lane `navigate` caught 94 percent, 66 of 70 case-runs, against full text's 96 percent, 67 of 70. The gate was followed and `full_text` became the deployed default. The question closed there, and the commitment recorded and signed before the next campaign ran says so in its own words: "the mode-selection question closed with the 08-25 campaign; the new campaign measures the shipping configuration and republishes both modes; the ship-gate output is recorded as informational, not as a decision input; the default remains full_text regardless of one-run noise in either direction." The second campaign, on 2026-08-25 at code revision `8900a76` and the same ten audits per pair, measured the shipping configuration and met all eight conditions, with the messy lane at `navigate` 90 percent, 63 of 70, against full text's 86 percent, 60 of 70. The ordering flipped by a few case-runs in seventy between two campaigns, which is run-to-run variance and not a capability gap. The default therefore remains `full_text`, this campaign's gate verdict is published in [EVAL.md](EVAL.md) line by line as informational, and `navigate` stays selectable and fully receipted. The two campaigns are never pooled into one statistic; every number on this page names the campaign that measured it.
...
- `EVAL-RECEIPTS.jsonl`: every model-initiated tool call of the published campaign, one JSON line per call, followed by one summary line per run.
- `scripts/reset_demo_ledger.py`: archive-then-clear for the demo ledger.
- `fixtures/`: seven fictional, generated, text-based PDFs and their manifest.
...
`uv run pytest -q` exited 0 with **681 passed** on code revision `9a6459d`. This line is written by `scripts/record_test_count.py` from that run's own summary line; it is not typed by hand. No test requires the network or credentials; every model, Firestore, and storage dependency is an in-process fake.

<!-- test-count-end -->

Deployed service: `https://specguard-108657628939.us-central1.run.app` (Cloud Run, us-central1, revision `specguard-00030-jnr` as of 2026-08-26). Every public GET route reads only, with one stated exception: `GET /` also mints one upload submission token and records it, within a per-address hourly budget. Work that costs something — an audit, a sample audit, a gate check — is a POST. `POST /audit` requires a demo passphrase that is not in this repository.
...
`specguard/integrity.py` reads the file's bytes once, into one immutable snapshot, and derives the SHA-256, the page count, and the flag evidence from that snapshot, so a replacement during a screen cannot make the hash describe one byte stream while the evidence describes another. It reads PyMuPDF span data and PDF content streams only. It renders no image, runs no OCR, and compares no pixels. Six deterministic rules run over every page, in a fixed order. Every flag names the rule that raised it and states in one line what that rule read, a flag from any rule quarantines the run on exactly the terms a render-mode-3 flag does, and the text of a flagged span is kept out of the page's readable text whichever rule raised it. The screen identity stored beside every record is `text_layer_integrity_v3`; `v1` was the render-mode-3 rule alone and `v2` was the five-detector set. The identity moves whenever the rule set moves, and a test ties the two together so it cannot go stale again.
...
The five original-lane fixtures are the parametrized false-positive check for all six rules: the four unaltered ones raise no flag at all and the altered fixture raises render-mode-3 flags and nothing else. Pinned by `test_no_committed_fixture_trips_a_new_detector`. The two messy-lane fixtures are not in that parametrization; they are screened on every audit, and no messy-lane run of the 2026-08-25 campaign was quarantined.
...
Measured on 2026-08-25 by `scripts/eval_fixtures.py`, 10 audits per document pair against the deployed Vertex AI model path, on code revision `8900a76`, in `full_text` mode. Catch rate: 89% across every planted discrepancy. Not every case matched its declared expected outcome. The tables above show the measured numbers, including the cases that missed. SpecGuard does not catch every planted discrepancy on every run. This run's ship gate met every condition, and that output is informational: the mode-selection question closed with the 2026-08-25 campaign, so `full_text` is the deployed default and no single campaign's gate output changes it. `navigate` stays selectable and receipted. EVAL.md publishes both modes and the gate verdict line by line. Column definitions and the full record are in [EVAL.md](EVAL.md).
...
Numbers moved from the 2026-08-25 run. These cases now measure differently: `E-13`, `E-15`. The table above is the current measurement; the earlier figures are superseded, not corrected.

<!-- eval-table-end -->

**Provenance of the current table.** The table above was regenerated on 2026-08-25 by the harness at code revision `8900a76`, over 100 real audits at ten iterations per document pair, in both modes and both lanes. It publishes `full_text`, the deployed default; [EVAL.md](EVAL.md) publishes all four mode-and-lane sections and the ship-gate verdict line by line. The measured revision is the described revision. Two earlier measurements are superseded by this campaign, not corrected by it: the first 2026-08-25 campaign at revision `31ef186`, and the 2026-08-23 campaign at five iterations on revision `1340748`, which also ran under a different scoring rule. Records from different campaigns are not comparable cell by cell and are never pooled into one statistic. The generated table states, in its own last line, which cases moved.

**Two campaigns ran on 2026-08-25, and the mode ordering flipped between them.** The first, at `31ef186`, measured messy-lane catch at `navigate` 94 percent (66 of 70 case-runs) against `full_text` 96 percent (67 of 70); the ship gate failed on that one condition and the deployed default was fixed to `full_text`. The second, at `8900a76`, measured the shipping configuration and reversed the order: `navigate` 90 percent (63 of 70) against `full_text` 86 percent (60 of 70), and the gate met all eight conditions. A few case-runs in seventy moved in each direction between two runs of the same fixtures, which is run-to-run variance rather than a capability gap. The pre-commitment quoted at the top of this file was recorded before the second campaign ran and holds: the default remains `full_text` and the second campaign's gate output is informational.

**Why `full_text` costs about two fifths less than it did.** The narrowed registration landed at `8900a76`: `full_text` now registers the same three read-only tools `navigate` registers, and neither of the two that write, so the model sees three tool schemas where it saw five. Its mean prompt tokens fell from 13,233 to 7,435 per run on the original lane and from 258,668 to 152,425 on the messy lane, against 305 model-initiated tool calls in the earlier campaign and 218 in this one. Fewer tool schemas and fewer tool round-trips are the receipted difference; no prompt text changed and no fixture changed. Both figures in each pair name their campaign: 13,233 and 258,668 are `31ef186`, and 7,435 and 152,425 are `8900a76`.

**What the receipts say about the self-check column, which reads 0 in both modes.** `EVAL-RECEIPTS.jsonl` holds 372 model-initiated `verify_quote` calls for this campaign, 178 in `full_text` and 194 in `navigate`, and every one of them answered that the quote was on the cited page. The column counts checks that answered the other way, so 0 here means no check rejected, not that nothing was counted: the receipts carry the answer per call, and the same counter recorded 53 rejections across the `full_text` lanes of the `31ef186` campaign. Neither number changed a claim's fate. The runtime gate ran on every claim regardless, and again before any write.

**What the receipts say about how the two modes read.** In this campaign `full_text` made no `extract_pdf_text` call at all and `navigate` made 295, which is the measured difference between the two reading paths rather than a description of it. The rest of the campaign's 748 recorded calls are 372 `verify_quote`, 80 `set_model_response` — the structured-output call ADK adds for this model, one per model turn — and one `check_text_integrity`. No call to either write tool appears anywhere in the file, in either mode.

**What moved since the 2026-08-23 run, and why.** Two deliberate changes landed before the `31ef186` campaign and both move numbers upward, so neither is a model improvement. The harness now scores a catch by evidence equality: the finding must cite the page the manifest records on each side, and the persisted quote and the manifest quote must contain one another in either direction after the gate's own normalization. The earlier rule required the persisted span to equal the planted span byte for byte, which scored a correct finding with a shorter quote as a miss and then counted the same record as an unattributed false positive. The `M-05` package row was also re-specified from a disclaimed recommendation to a flat characteristic of the product as furnished, which turned `E-15` from a case the prompt's own both-can-be-true exclusion covered into a case the model can fairly be asked to report. Both changes are receipted below, and both were already in place when the `8900a76` campaign ran. The severity column is unchanged in meaning: this campaign again executed with the severity sentinel (`SPECGUARD_GEMMA_ENDPOINT=disabled`), so every persisted finding carries `unclassified` with its recorded fallback reason.

**The run arithmetic.** Five document pairs at ten iterations each, in each of the two modes, is 100 audits. The altered fixture is quarantined before any model call, so its twenty runs make zero model turns; the other 80 runs make one model turn each. The messy lane scores its nine declared cases from each of its ten audits per mode, which is why 100 audits cover 26 case measurements.

**The messy-lane misses that remain.** `E-13` and `E-15` were published at 40 percent and 0 percent by the 2026-08-23 run. Phase 7d inspected both against the persisted records of those runs and found two different causes, one in the scorer and one in the planted text. Both causes were corrected before the two 2026-08-25 campaigns, and both cases now measure as real detection results rather than as artefacts. The inspection and every receipt behind it are in [REVIEW-7D-MISSES.md](REVIEW-7D-MISSES.md); it describes the 2026-08-23 run and is not a record of either later one.

- `E-13` was a scoring artefact and is now measured as a catch. Under the old rule six of ten runs that found the discrepancy on the correct pages were scored as misses for returning a shorter quote span. Under evidence equality the same behaviour scores as what it is: in the `8900a76` campaign `E-13` measures 100 percent in both modes, across the ten runs of each.
- `E-15` was a fixture-design finding and is now a real test the model often fails. No run in either mode had ever raised that pair, because the package stated its clearance figure as a recommendation while both prompts tell the model not to report statements that can both be true. The package row now states a flat characteristic of the product as furnished, which is the shape of the six pairs that already caught. Nothing else on that page changed, and no prompt changed. In the `8900a76` campaign `E-15` measures 0 percent in `full_text` and 30 percent in `navigate`; in the `31ef186` campaign it measured 80 percent and 60 percent. It is the lowest case on the board in both campaigns and the only case that misses in either, and it is published as measured.

**The unattributed false-positive column is zero in all four sections.** The 2026-08-23 run reported 16 of them on the messy lane, 13 in `full_text` and 3 in `navigate`. An offline classification of those records showed that every one reproduced a planted pair on the correct two pages with a different quote span, and that none was an invention. Evidence-equality scoring attributes those findings to the cases they belong to, and this campaign measured zero unattributed false positives and zero decoy false positives in every lane and mode. That result is now a harness measurement rather than an offline classification.

Two things these numbers do not show. The gate rejected nothing and the runtime retried nothing in the 80 model-calling runs, because the model cited every quote correctly on the first turn, so this table is not evidence that the rejection-and-retry loop works; `tests/test_agent.py` and `tests/adversarial/test_runtime_separation.py` drive rejections deterministically and prove the loop. Severity is advisory and a fallback never blocks an audit, so a run with the endpoint down publishes `unclassified` labels and says why on each finding. The measurement covers seven fictional fixtures, not a corpus of real submittals; it is not evidence of accuracy on documents outside this set.
...
The generated RFI draft carries a header block, a findings table, the text-layer screen result for each document, reviewer signature lines, and, past those signature lines in reduced type, the chain-of-custody block: the hex run identifier and the full SHA-256 of each source document under the caption that calls them chain-of-custody metadata only. The header block names the RFI number, the submittal number it answers, the project, the owner, the date, and the finding count, and carries no hash and no hex identifier; a reviewer's first screen is findings, quotes, and pages. No digest is shortened anywhere — a truncated hash has no custody value. The gate re-verifies every quote in the draft before it renders. Claim and quote text is model-generated and has no maximum length, so a table row taller than one page is written in slices under a repeated header rather than clipped at the page edge.
...
- The four original demo fixtures produce zero flags, and the altered fixture raises render-mode-3 flags and nothing else. That is the false-positive check, run over those five fixtures and all six detectors.
- Each of the five detectors added after render mode 3 has a generated page it must flag and a near-miss page it must leave alone. The near-misses are a span the crop edge cuts through, a span at alpha 0.2, painted mode-4 text and text clipped by an ordinary path, a glyph at exactly 1.0 pt, and four soft-mask constructs that leave text visible.
...
| P3 | No receipt proves a model initiated a registered tool call. | FIXED — `3c09dc5` records every model-initiated call (`model_tool_calls`) with tool, arguments, and turn; measured across 100 audits in EVAL.md; the runtime still owns every write. |
...
| 7d review | The `soft_mask_hidden` rule read only an inline `/SMask` dictionary, so a mask written as an indirect reference hid text with nothing flagged. | FIXED — both forms are resolved; pinned by `test_a_soft_mask_reached_through_an_indirect_reference_is_flagged`. |
| 7d review | An ExtGState carrying no `/SMask` entry cleared the active soft mask, so a second `gs` naming other parameters let masked text through. | FIXED — an absent entry now leaves the mask in force, and only `/SMask /None` or a replacement clears it; pinned by `test_an_extgstate_without_an_smask_leaves_the_active_mask_in_force` with the near-miss `test_an_explicit_smask_none_still_clears_the_active_mask`. |
| 7d review | Page resources inherited from a `/Pages` node were not read, so no `gs` name resolved on a page that omits its own `/Resources`. | FIXED — the screen walks the `/Parent` chain under a depth bound; pinned by `test_an_extgstate_inherited_from_the_page_tree_is_resolved`. |
| 7d review | A luminosity mask group that painted nothing was read as black, so a page whose mask carries a white `/BC` backdrop was quarantined although its text is visible. | FIXED — an unpainted group takes its `/BC` backdrop, defaulting to the black the specification names where `/BC` is absent or unreadable, so the fix closes a false quarantine without opening a way past the rule; pinned by `test_a_white_backdrop_over_an_empty_mask_group_is_not_flagged`, `test_a_black_backdrop_over_an_empty_mask_group_is_flagged`, and `test_an_absent_backdrop_over_an_empty_mask_group_is_flagged`. |
| 7d review | A soft-mask flag contributed no span identity, so the masked line stayed in the page report's `visible_text` where every other detector removes it. | FIXED — the masked spans are excluded from readable text; pinned by `test_soft_masked_text_is_kept_out_of_the_readable_text` with the near-miss `test_a_visible_soft_masked_page_keeps_its_text_readable`. No model ever read that field: the model-facing tool returns the flag summary and no text. |
| 7d review | `SCREEN_ID` still read `text_layer_integrity_v2` after the sixth detector landed, so a record written with soft-mask coverage was indistinguishable from one written without it. | FIXED — bumped to `text_layer_integrity_v3`, and `test_the_screen_identity_names_the_widened_rule_set` now ties the version to the detector count so it cannot go stale again. |
...
| Review 7a | The published evaluation names `a424ccf`, and Phase 7a changed files inside the measured path. | FIXED — the harness re-ran on 2026-08-23 at `1340748`, and again on 2026-08-25 at `31ef186`, both modes and both lanes, with the severity sentinel disabled and every fallback reason recorded; the table above is the 2026-08-25 run. |
| 7c | `E-15` (0 of 5) and `E-13` (2 of 5) were published misses on the messy lane in both modes. | CLOSED — both causes were corrected and both cases re-measured at N=10 in the two 2026-08-25 campaigns. `E-13` was a scoring artefact: under evidence-equality scoring it measured 100% in `navigate` and 90% in `full_text` at `31ef186`, and 100% in both modes at `8900a76`. `E-15` was a fixture-design finding: against a package row that states a flat product characteristic it measured 80% in `full_text` and 60% in `navigate` at `31ef186`, and 0% and 30% at `8900a76`. `E-15` remains the lowest case on the board and publishes as an honest miss on a now-fair test; disclosed in [Measured evaluation](#measured-evaluation). The earlier inspection stays receipted in `REVIEW-7D-MISSES.md`, which describes the superseded 2026-08-23 run. |
| 7d | Under the exact-span rule a finding that reproduces a planted pair on the correct pages with a different quote span scored as a miss and was then counted as an unattributed false positive. | CLOSED — `_matches_pair` now scores by evidence equality: the cited page must match and the two quotes must contain one another in either direction after `specguard.gate.normalize`, on the gate's own token boundaries. Seven known-good and known-bad tests pin the rule. Every number on this page was measured under it, and the unattributed column is zero in all four sections. |
| 7d | The disclosure paragraphs added to `EVAL.md` in the 7d-eval phase were hand-written, and `scripts/eval_fixtures.py` rewrites that file in full on every regeneration. | CLOSED — the 2026-08-25 regeneration replaced them, exactly as the row predicted. `EVAL.md` now carries no hand-written section: the provenance, supersession, and self-check-rule statements live in the generator and are pinned by tests, so the next regeneration re-emits them. The earlier explanations remain in `REVIEW-7D-MISSES.md` and `HANDOFF.md`, which the harness does not write. |
| 7c | `full_text` mode registers the two write tools, `persist_finding` and `draft_rfi`, to the model, and `full_text` is now the deployed default. | CLOSED — applied at `8900a76`: `full_text` now registers the same three read-only tools `navigate` registers and neither of the two that write. `tests/test_navigate_mode.py::test_both_modes_expose_one_identical_read_only_model_facing_surface` pins the two registrations as identical, so the parity cannot drift back. The `8900a76` campaign is the receipt the earlier row said was missing: `EVAL-RECEIPTS.jsonl` records all 748 model-initiated calls of its 100 audits, and no call to `persist_finding` or `draft_rfi` appears in either mode. The runtime still owns every write and every write path runs the gate at write time and again before the write. |
| 7d | A CLI eval campaign persisted no tool-call receipt: the harness writes no `runs` document, so the `model_tool_calls` the runtime recorded lived only in harness memory and were gone when the process exited, leaving the published tool-call columns uncheckable. | CLOSED — `b57c35a` writes `EVAL-RECEIPTS.jsonl` during the campaign and `d98e813` commits it as provenance: one JSON line per model-initiated call carrying mode, lane, document pair, iteration, run identifier, turn index, tool name and the bounded arguments the runtime recorded, followed by one summary line per run. The published tool-call, self-check, and per-mode reading figures can now be recomputed from that file call by call. |
| 7d | The `8900a76` campaign's ship gate passed all eight conditions, which would have moved the deployed default to `navigate` on one campaign's numbers. | CLOSED — 2026-08-25. The mode question had already closed with the first 2026-08-25 campaign, and the pre-commitment recorded before this one ran fixed the handling in advance: the ship-gate output is recorded as informational, not as a decision input, and the default remains `full_text` regardless of one-run noise in either direction. The gate is still evaluated and still published condition by condition in `EVAL.md`; a `DecidedDefault` value in `scripts/eval_fixtures.py`, carrying the mode and the date the decision closed, now selects the mode the README publishes, and tests pin both the informational wording and the fact that the renderer follows that value rather than the verdict. |
| 7c | The EVAL self-check totals were measured under the digest-counting rule. | CLOSED — the 2026-08-25 regeneration is the first measurement taken under the corrected call-and-anchor rule, and `EVAL.md` says so in its own text. No total on this page was measured under the digest rule. |
