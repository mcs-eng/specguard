...
`uv run pytest -q` exited 0 with **409 passed** on code revision `6f27d92 plus uncommitted changes to scripts/record_test_count.py, tests/test_record_test_count.py`. This line is written by `scripts/record_test_count.py` from that run's own summary line; it is not typed by hand. No test requires the network or credentials; every model, Firestore, and storage dependency is an in-process fake.

<!-- test-count-end -->

Deployed service: `https://specguard-108657628939.us-central1.run.app` (Cloud Run, us-central1, revision `PLACEHOLDER_REVISION` as of 2026-08-22). Every public GET route reads only, with one stated exception: `GET /` also mints one upload submission token and records it. Work that costs something — an audit, a sample audit, a gate check — is a POST. `POST /audit` requires a demo passphrase that is not in this repository.
...
**What the published revision covers, and what it no longer does.** The table names code revision `a424ccf`, which is the revision that ran. Code has changed since, in two distinct groups, and the second group breaks the byte-identity claim this paragraph used to make.

*Before Phase 7a.* Six commits changed seven files under `specguard/`. Every one is web-layer or `_RfiWriter`: `web/app.py`, `web/repository.py`, and the four templates, plus the RFI table pagination inside `specguard/tools.py`. None can move a number in the table. Every counter is fixed before `draft_rfi` renders a page, and no column is read from the web service. Across that range the gate, the integrity screen, the agent runtime, the severity classifier, and the persistence tools were byte-identical to the revision measured.

*Phase 7a.* Eight more commits changed `specguard/agent.py`, `specguard/models.py`, `specguard/severity.py`, and `specguard/tools.py` as well, so that byte-identity list no longer holds and this paragraph does not repeat it. What changed inside the measured path is narrow and named: the severity annotation refuses a finding that is not a verified record of the same run; `extract_pdf_text` and `check_text_integrity` report an unreadable document as data instead of raising, and carry no message text; severity provenance splits into an observed `severity_model_id` and a configured `severity_endpoint_label`; and the RFI findings heading is renamed. None of these paths is reached by a run over the five committed fixtures, which are readable, produce verified findings, and are annotated exactly as before, so the measured counters have no route by which to move. That is a reasoned claim about the code, not a receipt.

**This table has not been re-measured against Phase 7a.** The Vertex endpoint is torn down outside demo windows, so re-running the harness needs a GPU deployment. Two things would read differently on the next run: `EVAL.md` names the severity model measured from the persisted findings, and that value now comes from the endpoint's own response rather than from the configured label. The catch rate, false positives, rejections, retries, quarantine rate, and model calls are expected to be unchanged. The board carries this as an open item.
...
The run arithmetic. Four cases at five iterations each is 20 runs. `E-04` is the altered fixture: the integrity screen quarantines it before any model call, so those five runs make zero model calls. The other three cases make one model call each, which is 15. That is where "20 runs" and "15 model-calling runs" both come from, and the model-calls column adds to the same 15.
...
| Review 7a | The published evaluation names `a424ccf`, and Phase 7a changed files inside the measured path. | ACCEPTED — the affected paths are not reached by a run over the five committed fixtures, and re-measuring needs a GPU endpoint deployment; the next shoot-day window regenerates the table; disclosed in [Measured evaluation](#measured-evaluation). |
| Review 7a | README stated a hand-typed test count. | FIXED — `PLACEHOLDER_ITEM10` generates the count and its revision from a real `uv run pytest -q` receipt; disclosed in [What is in this repository](#what-is-in-this-repository). |
| Review 7a | README called the web repository read-only, and its diagram called the severity annotation "never a write path". | FIXED — `PLACEHOLDER_ITEM10` states that the repository writes runs, tokens, and rate-limit counters but no claim record, and that the severity annotation writes severity fields and cannot change a verification status. |
