...
| Review 7a | `POST /audit` accepted any string as a submission token, because none was ever recorded. | FIXED — `PLACEHOLDER_ITEM4` mints every token server-side on page render and records it with a one-hour expiry; an unminted or expired token is refused in the route and again in the create-run transaction; disclosed in [Service limits](#service-limits). |
