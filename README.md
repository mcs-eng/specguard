...
The agent proposes; the runtime disposes. One Google ADK agent on Gemini 3.7 Flash reads the two documents and returns discrepancy claims, each with one verbatim quote and one page number per document. A deterministic runtime around the agent screens both documents before the model reads anything, verifies every quoted anchor, sends a rejected claim back to the model once, verifies again at write time, and drafts the RFI. The runtime has two modes, selected by `SPECGUARD_AGENT_MODE`. In `navigate`, the deployed default, the model receives the submitted document in full plus a one-line-per-page index of the specification, reads specification pages through read-only tools, and every model-initiated tool call is recorded on the run and shown on its page. In `full_text`, the code default for the library path, the model receives every page of both documents up front. In both modes the runtime owns every write: navigate registers no write tool to the model, and the gate verifies every claim whatever the model called. Navigation costs more prompt tokens than full text and is the default for two reasons, neither of which is cost. It scored a higher catch rate on the hardest fixture lane under the harness's exact-span rule, 77 percent against 49 percent. It also returned the exact planted quote span far more often: an offline classification of the persisted findings from those same runs counts 27 exact spans of 30 in navigate against 17 of 30 in full text, and 3 span variants against 13. The second reason qualifies the first, because most of the catch-rate gap is quote-span discipline rather than discrepancies full text failed to find. That classification is receipted in [REVIEW-7D-MISSES.md](REVIEW-7D-MISSES.md) and is not a harness measurement. [EVAL.md](EVAL.md) publishes both modes.
...
**The two published messy-lane misses.** `E-13` and `E-15` are published above as measured, at 40 percent and 0 percent. Phase 7d inspected both against the persisted records of the ten messy-lane runs and found two different causes. Neither number changed, and no run was repeated. The inspection and every receipt behind it are in [REVIEW-7D-MISSES.md](REVIEW-7D-MISSES.md).

- `E-13` is a scoring artefact, not a detection failure. All ten messy-lane runs, five per mode, persisted a verified finding for that discrepancy citing the correct page in each document. Six of the ten returned a shorter package quote than the one the manifest plants, and the harness requires the exact planted span, so those six scored as misses. The runtime found the discrepancy in 10 of 10 runs. Whether evidence equality rather than span equality is the honest definition of a catch is a real question about the metric, and it is recorded as an open follow-up rather than settled here.
- `E-15` is a fixture-design finding. No run in either mode raised that pair, no run cited the governing package page, and the gate recorded no rejection for it, so it was never attempted rather than attempted and refused. Both governing pages were in front of the model in every navigate run: two other quotes came back verbatim from the same specification page, and the page-index line is too short to have supplied either, so the page was opened through the read-only tool. The package states its clearance figure as a recommendation, under a heading calling its dimensions nominal and a closing line directing that set-out dimensions be coordinated with the project room. Both prompts tell the model not to report statements that can both be true, and a disclaimed recommendation alongside a project requirement is such a pair. The model followed the instruction it was given.

**Every unattributed false positive above is a span variant, not an invention.** The unattributed column counts a persisted finding that matched no planted pair and no decoy pair under the exact-span rule. All 16 such findings across the messy lane, 13 in `full_text` and 3 in `navigate`, reproduce a planted pair on the correct two pages with a different quote span. Across all 60 persisted messy-lane findings there are zero inventions and zero decoy hits. The column definition is unchanged and the counts stand as measured; this states what the records show they contain.
...
| 7c | `E-15` (0 of 5) and `E-13` (2 of 5) are published misses on the messy lane in both modes. | ACCEPTED, now explained — Phase 7d inspected both against the persisted records of all ten messy-lane runs. `E-13` is a scoring artefact: the runtime found the discrepancy in 10 of 10 runs and the harness requires the exact planted span. `E-15` is a fixture-design finding: no run raised the pair, and the package states its clearance as a disclaimed recommendation, which the prompt's both-can-be-true exclusion covers. Both numbers stand as measured; disclosed in [Measured evaluation](#measured-evaluation) and receipted in `REVIEW-7D-MISSES.md`. |
| 7d | Under the exact-span rule a finding that reproduces a planted pair on the correct pages with a different quote span scores as a miss and is then counted as an unattributed false positive. | OPEN — the metric-definition question, whether evidence equality is the honest definition of a catch, is deferred to a post-submission decision. `_matches_pair` is unchanged in this phase. The published counts stand as measured, and what the 16 such findings actually contain is disclosed in [Measured evaluation](#measured-evaluation). |
| 7d | The disclosure paragraphs added to `EVAL.md` in this phase are hand-written, and `scripts/eval_fixtures.py` rewrites that file in full on every regeneration. | OPEN — no regeneration ran in this phase, so the text stands. A future regeneration drops it unless the prose is moved into the generator or reapplied; recorded in `HANDOFF.md` Phase 7d-eval. The README disclosure sits outside the generated markers and survives. |
