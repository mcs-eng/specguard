...
`uv run pytest -q` exited 0 with **414 passed** on code revision `7283956 plus uncommitted changes to specguard/severity.py, specguard/web/app.py, specguard/web/repository.py, specguard/web/templates/index.html, tests/test_severity.py, tests/test_web.py`. This line is written by `scripts/record_test_count.py` from that run's own summary line; it is not typed by hand. No test requires the network or credentials; every model, Firestore, and storage dependency is an in-process fake.
...
| 7a review | Minting a token on every public read was an unbounded Firestore write surface. | FIXED — `PLACEHOLDER_FIX` mints nothing for a `HEAD` probe and caps mints at 30 per address per UTC hour; over budget the page renders without the upload form; a Firestore TTL policy on `expires_at` removes spent records; disclosed in [Service limits](#service-limits). |
| 7a review | Filtering sample runs after the query limit could empty the public list. | FIXED — `PLACEHOLDER_FIX` reads the newest 100 runs and keeps the newest 20 sample ones; disclosed in [Who can read a run](#who-can-read-a-run). |
| 7a review | The generativelanguage backend still reported its configured request name as `severity_model_id`. | FIXED — `PLACEHOLDER_FIX` reads that response's reported model version and records the request name as the endpoint label, so one rule covers both backends. |
