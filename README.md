...
- **A span masked to nothing by a soft mask.** An ExtGState soft mask (`/SMask`) can drive a text span's effective opacity to zero from outside the span, so the painted page shows nothing while extraction still returns the characters and the span's own `alpha` reads 255. The `zero_alpha` rule reads span alpha, so it does not see this. Found and measured in Phase 7b-redteam; pinned by `tests/test_integrity_redteam.py::test_the_two_known_misses_are_still_missed`. A content-stream soft-mask detector is proposed as a follow-up, not built here, because it needs `ExtGState` tracking in the render-mode scan and a false-positive pass over honest documents that use soft masks on images.
...
| P7b-redteam | A soft mask (`/SMask`) can drive a text span's opacity to zero from the graphics state, which the span-alpha rule cannot see. | ACCEPTED for now, detector proposed as a follow-up in PLAN.md — it needs ExtGState tracking in the render-mode scan and a false-positive pass; measured and disclosed in [Not detected (and why)](#not-detected-and-why), pinned by `tests/test_integrity_redteam.py::test_the_two_known_misses_are_still_missed`. |
| P7b-redteam | Eleven concealment mechanisms across the PDF imaging model were measured against the screen. | FIXED — `tests/test_integrity_redteam.py` pins extraction, rendered visibility, and screen result for each, so the Detected / Not detected list is now a receipt rather than a claim; two real misses (soft mask, white text), the rest detected or non-threats where extraction returns nothing. |
