...
Measured on 2026-08-22 by `scripts/eval_fixtures.py`, 5 runs per case against the deployed Vertex AI model path, on code revision `616e9f1
 plus uncommitted changes`. Catch rate: 100% across every planted discrepancy. Every case matched its declared expected outcome in every run. Column definitions and the full record are in [EVAL.md](EVAL.md).
...
| P6e review | Every run page GET downloads and reparses both stored PDFs, with no cache and no rate limit. | ACCEPTED — run identifiers are public, so repeated GETs repeat that work; the service caps instances at one and concurrency at two, and a document it cannot read yields no window and says so. A cache is the fix if this ever costs anything. |
| P6e review | A findings-table row taller than one RFI page is clipped, not split. | ACCEPTED — a claim or quote would need roughly sixty wrapped lines to fill a page; measured fixture rows run four to six. The row moves to a fresh page but is not sliced across two. |
