...
- Narration [40]: "A hundred real runs on the fixtures, against a named code revision. Both planted discrepancies on the original lane caught in every run of both modes, no false positive on the compliant cut sheet, no decoy hit anywhere. On the hard lane, ninety percent in navigate against eighty-six in full text — three case-runs in seventy, which is noise, not a winner. And every tool call behind those numbers is committed, one line each."
